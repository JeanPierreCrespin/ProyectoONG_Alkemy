...
import alkemy.challenge.Challenge.Alkemy.config.SengridConfig;
import alkemy.challenge.Challenge.Alkemy.controller.dto.ResponseDto;
import alkemy.challenge.Challenge.Alkemy.controller.request.AuthenticationRequest;
...
public class AuthenticationController {
	...
	@Autowired
	public AuthenticationController (UserService userService, AuthenticationManager authenticationManagerBean,MyUserDetailsService myUserDetailsService, JwtUtil jwtTokenUtil, SengridConfig sengridConfig, PasswordEncoder passwordEncoder) {
	...
	public ResponseEntity<?> createUser(@RequestBody UserRequest userRequest){ 
			...
			if (userRequest.getFirstName() == null || userRequest.getLastName() == null
				...
				return new ResponseEntity<>(new ResponseDto(400,"firstname, lastname, password and email are required."),
					...
					authenticationManagerBean.authenticate(new UsernamePasswordAuthenticationToken(userRequest.getEmail(),userRequest.getPassword()));
				...
				try {
					...
					return new ResponseEntity<>(new ResponseDto(200, "Failed to send email to: "  + user.getEmail() +"  Token: " +jwt),
							HttpStatus.OK);
				}
				return new ResponseEntity<>(new ResponseDto(200, "Id: " + user.getId() +"   Token: " +jwt), HttpStatus.OK);
			}
	}
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "500", description = "    Internal Server Error") })
	@PostMapping("/login")
	public ResponseEntity<?> createAuthentication(@RequestBody AuthenticationRequest authReq) throws BadCredentialsException{
			...
			if (authReq.getUsername() != null && authReq.getPassword() != null) {
				...
				if (userDetails != null) {
					...
					if (passwordEncoder.matches(authReq.getPassword(), userDetails.getPassword())) {
						...
						authenticationManagerBean.authenticate(new UsernamePasswordAuthenticationToken(
				...
				return new ResponseEntity<>(new ResponseDto(400,"Password and Username are required"), HttpStatus.BAD_REQUEST);
			}
	}
